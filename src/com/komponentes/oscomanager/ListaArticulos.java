...
public class ListaArticulos extends Activity implements OnClickListener{
	...
	*/
	...
	public void onClick(View v) {
		switch (v.getId()) {
			case R.id.buttonEstado:
				OperacionAccesoRed obtenerListaArticulos = new OperacionAccesoRed();
				obtenerListaArticulos.execute();
				break;
			case R.id.button2:
				finish();
				break;
    ...
    public void pintaLista(String response){
		...
		String[] from = new String[] {"articulo", "precio"};
	    ...
	    if (response != null){
		    ...
		    try {
		    	...
		    	JSONArray jsonArray = jsonMessages.getJSONArray("articulos");
		    	for (int i = 0; i < jsonArray.length(); i++) {
			        JSONObject jsonObject = jsonArray.getJSONObject(i);
			        precio.add(precioToEur(Double.parseDouble(jsonObject.getString("products_price"))));
			        JSONArray jsonArrayProducts = jsonObject.getJSONArray("products_description");
			        articulos.add(jsonArrayProducts.getJSONObject(1).getString("products_name"));
		...
		// Add each String,String element to a new HashMap with keys: "articulo", "precio"
		for (int i=0; i<articulos.size(); i++) {
			HashMap<String, String> map = new HashMap<String, String>();
			map.put("articulo", articulos.get(i));
			map.put("precio", precio.get(i));
			pedidos.add(map);
		}
...
//                startActivity(i);
	...
	private String precioToEur(double precio){
		//return String.valueOf(precio);
		return (new DecimalFormat("#0.00").format(precio) + " EUR");
	}
}
